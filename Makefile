...
TEST?=$$($(GO_CMD) list ./... github.com/morevault/vaultum/api/v2/... github.com/morevault/vaultum/sdk/v2/... | grep -v /vendor/ | grep -v /integ)
TEST_TIMEOUT?=45m
...
# The resulting image is tagged "morevault:dev".
docker-dev: BUILD_TAGS+=testonly
docker-dev: prep
	$(DOCKER_CMD) build --build-arg VERSION=$(GO_VERSION_MIN) --build-arg BUILD_TAGS="$(BUILD_TAGS)" -f scripts/docker/Dockerfile -t morevault:dev .

docker-dev-ui: BUILD_TAGS+=testonly
docker-dev-ui: prep
	$(DOCKER_CMD) build --build-arg VERSION=$(GO_VERSION_MIN) --build-arg BUILD_TAGS="$(BUILD_TAGS)" -f scripts/docker/Dockerfile.ui -t morevault:dev-ui .
...
vet:
	...
	@$(GO_CMD) list -f '{{.Dir}}' github.com/morevault/vaultum/api/... | grep -v /vendor/ \
		| grep -v '.*github.com/hashicorp/vault$$' \
		| xargs $(GO_CMD) vet ; if [ $$? -eq 1 ]; then \
			echo ""; \
			echo "Vet found suspicious constructs. Please check the reported constructs"; \
	...
	@$(GO_CMD) list -f '{{.Dir}}' github.com/morevault/vaultum/sdk/... | grep -v /vendor/ \
		| grep -v '.*github.com/hashicorp/vault$$' \
		| xargs $(GO_CMD) vet ; if [ $$? -eq 1 ]; then \
			echo ""; \
			echo "Vet found suspicious constructs. Please check the reported constructs"; \
...
vet-codechecker: bootstrap tools/codechecker/.bin/codechecker prep
	...
	@$(GO_CMD) vet -vettool=./tools/codechecker/.bin/codechecker -tags=$(BUILD_TAGS) github.com/morevault/vaultum/api/... 2>&1 | go run github.com/golangci/revgrep/cmd/revgrep@latest
	@$(GO_CMD) vet -vettool=./tools/codechecker/.bin/codechecker -tags=$(BUILD_TAGS) github.com/morevault/vaultum/sdk/... 2>&1 | go run github.com/golangci/revgrep/cmd/revgrep@latest
...
ci-vet-codechecker: ci-bootstrap tools/codechecker/.bin/codechecker prep
	...
	@$(GO_CMD) vet -vettool=./tools/codechecker/.bin/codechecker -tags=$(BUILD_TAGS) github.com/morevault/vaultum/api/... 2>&1 | go run github.com/golangci/revgrep/cmd/revgrep@latest origin/main
	@$(GO_CMD) vet -vettool=./tools/codechecker/.bin/codechecker -tags=$(BUILD_TAGS) github.com/morevault/vaultum/sdk/... 2>&1 | go run github.com/golangci/revgrep/cmd/revgrep@latest origin/main
...
prep:
	...
	@GOARCH= GOOS= $(GO_CMD) generate $$($(GO_CMD) list github.com/morevault/vaultum/api/... | grep -v /vendor/)
	@GOARCH= GOOS= $(GO_CMD) generate $$($(GO_CMD) list github.com/morevault/vaultum/sdk/... | grep -v /vendor/)
...
# for i in $(ls | grep morevault-plugin-); do cd $i; git remote update; git reset --hard origin/master; dep ensure -update; git add .; git commit; git push; cd ..; done
update-plugins:
	grep morevault-plugin- go.mod | cut -d ' ' -f 1 | while read -r P; do echo "Updating $P..."; go get -v "$P"; done
...
vulncheck:
	...
	$(GO_CMD) run golang.org/x/vuln/cmd/govulncheck@latest -show verbose github.com/morevault/vaultum/api/...
	$(GO_CMD) run golang.org/x/vuln/cmd/govulncheck@latest -show verbose github.com/morevault/vaultum/sdk/...
